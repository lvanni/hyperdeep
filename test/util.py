...
def build_dico_from_occ(repo, filename_dico, output_dico):
	...
	for i in range(4):
		for elem in dico_occ[i]:
			if dico_occ[i][elem] >= len_data[i] and elem not in dico[i]:
				dico[i][elem] = index[i]
				index[i] += 1
...
if __name__=="__main__":
	...
	filenames = ['Hollande.txt', 'Sarkozy.txt']
	repo = './data/dico'
	filename_dico ='occ_dico_H_S'
	...
	occ_dico = build_dictionnary(repo, filenames, filename_dico)
	embedding_dico = build_dico_from_occ(repo, filename_dico, output_dico)
	...
	print "\t", len(embedding_dico[0]), "formes"
	print "\t", len(embedding_dico[1]), "lemmes"
	print "\t", len(embedding_dico[2]), "codes"
	print "\t", len(embedding_dico[3]), "fonctions"
