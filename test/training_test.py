...
"""
def pre_process(repo, output_dico, filenames):
	index = 0
	y_value = []
	x_value = []
...
def training(dwin, x_train, x_valid, x_test, y_train, y_valid, y_test):
	...
	for epoch in range(epochs):
		...
		print "Test cost : " + str(np.mean(test_cost)*100)
		index_filename += 1
...
if __name__=='__main__':
	...
	x_train, x_valid, x_test, y_train, y_valid, y_test = pre_process(repo, output_dico, filenames)
	
	# Training, Validation, Test
	training(dwin, x_train, x_valid, x_test, y_train, y_valid, y_test)
