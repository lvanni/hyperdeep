...
class LookUpTable(Initializable, Feedforward):
	...
	input_ = input_.flatten()
    	W, = self.parameters
    	result, _ = theano.scan(fn=lambda x_i,W : W[:,x_i],
    		sequences=input_, non_sequences=W)
...
class Window(Initializable, Feedforward):
	...
	self.tables = [LookUpTable(self.vect_size[i], self.n_mot[i], weights_init=IsotropicGaussian(0.001), use_bias=False) for i in range(self.n_tables)]
    	self.mlp = MLP(activations=[Tanh()]*len(self.n_hidden)+[Identity()],
    		dims=[self.dwin*sum(self.vect_size)]+ self.n_hidden + [self.n_out],
    		weights_init=IsotropicGaussian(0.001), biases_init=Constant(0.))
        self.parameters = []
...
class LookUpTrain(Initializable, Feedforward):
	...
	if isinstance(n_hidden, int):
		...
		self.n_hidden = n_hidden
        self.n_out = n_out
    	self.window = Window(self.dwin, self.n_mot, self.vect_size, self.n_hidden, self.n_out,
